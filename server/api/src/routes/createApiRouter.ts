...
export function createApiRouter(telemetry: TelemetryService, logArchive: LogArchiveService): Router {
  ...
  const registry = new NodeRegistryService(telemetry.getDb(), saveDb)
  ...
  const commandController = new CommandController(commandService, registry, logArchive, auth, saveDb)
  ...
  const ingestController = new IngestController(telemetry, logArchive, registry, automationEvaluator)
  ...
  const provisionCtrl = new ProvisionController(registry, pairing, auth)
  ...
  router.delete('/nodes/:id', provisionCtrl.delete)
